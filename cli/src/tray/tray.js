...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `巡检服务 · 端口 ${port}`, tooltip: "服务状态", enabled: false },
    { title: "打开巡检系统", tooltip: "在浏览器打开", enabled: true },
...
/** macOS/Linux:systray2 惰性安装到 ~/.inspection-visualizer/runtime/node_modules */
function resolveSystray() {
  const runtimeNM = path.join(os.homedir(), ".inspection-visualizer", "runtime", "node_modules");
...
function chmodTrayBin(pkgName) {
  ...
  try {
    ...
    const candidates = [
      path.join(os.homedir(), ".inspection-visualizer", "runtime", "node_modules", pkgName, "traybin", binName),
...
function initUnixTray(options) {
  ...
  try {
    ...
    if (!SysTray) {
      process.stderr.write("[iv] macOS/Linux 托盘需要 systray2: npm i -g systray2 或重新运行安装\n");
      return null;
    }
    ...
    if (trayInstance.ready) {
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[iv] 托盘启动失败: ${err && err.message ? err.message : err}\n`);
      });
    }
    return trayInstance;
  } catch (err) {
    process.stderr.write(`[iv] 托盘初始化失败: ${err.message}\n`);
    return null;
  }
}
