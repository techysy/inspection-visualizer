...
if (-not (Test-Path (Join-Path $build "python\python.exe"))) {
    ...
    & (Join-Path $root "build-python-runtime.ps1") -PythonVersion "3.11.9" -Proxy $Proxy
...
robocopy $repo $appDir /E `
    /XD venv __pycache__ .git log backup node_modules desktop chrome-extension docs film_price_tracker integrations .tmp-data .github `
    /XF *.db *.log *.pid .env .gitignore .gitattributes start.bat iv-start.ps1 | Out-Null
