...
- 📊 **仪表盘解析**：自动识别监控/车辆/工牌三种仪表盘截图，提取结构化指标
- ⚙️ **指标配置**：为每个巡检对象配置需要跟踪的指标（名称、单位），可单独开关是否参与图表可视化
- 📦 **对象管理**：管理巡检对象，支持位置、类型分类，支持一键复制
...
│   ├── upload.html         # 截图识别（粘贴/拖拽/上传+对象匹配+位置标签）
...
| 类型 | 识别关键词 | 提取指标 |
|---|---|---|
| 📹 监控 | 监控点总数、监控点在线率 | 总数、在线、离线、未检测、在线率 |
| 🚗 车辆 | 全部车辆、车辆总数 | 总数、在线、报警（支持 "位置名（在线/总数）" 格式） |
| 🪪 工牌 | 工牌、未用 | 全部、在线、离线、未用 |

匹配时自动按设备类型加分，避免监控仪表盘误匹配到车辆对象。
...
- 🏷️ 位置标签：截图识别结果卡片左上角显示位置名称，便于确认

**🔧 修复与优化：**
- 🐛 修复巡检结果趋势图 tooltip 同时显示所有数据集的问题（改为 nearest 模式）
- 🐛 修复趋势图 x 轴日期不显示的问题（时间戳改为毫秒数）
- 🐛 修复指标趋势图鼠标靠近不显示 tooltip 的问题
- ⚡ 优化 tooltip 显示：去除重复标签，指标趋势图显示完整时间
- 🐛 修复车辆仪表盘误识别问题（使用具体标签关键词而非泛匹配）
